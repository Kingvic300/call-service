...
export class MeetingsGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
  ...
  onRaiseHand(@ConnectedSocket() client: Socket, @MessageBody() body: unknown) {
    return respond(() => {
      ...
      this.broadcaster.emitToMeeting(NAMESPACE, meeting.id, ServerEvent.HAND_RAISED, {
  ...
  onLowerHand(@ConnectedSocket() client: Socket, @MessageBody() body: unknown) {
    return respond(() => {
      ...
      this.broadcaster.emitToMeeting(NAMESPACE, meeting.id, ServerEvent.HAND_LOWERED, {
        peerId: participant.userId,
      });
      return { lowered: true };
    });
  ...
  onLowerParticipantHand(@ConnectedSocket() client: Socket, @MessageBody() body: unknown) {
    return respond(() => {
      const dto = validateWsPayload(TargetPeerDto, body);
      const meeting = this.signaling.getMeetingForNamespace(dto.meetingId, NAMESPACE);
      const actor = this.signaling.requireParticipant(meeting, client);
      this.moderation.lowerHand(meeting, { role: actor.role, userId: actor.userId }, dto.targetUserId);
      return { lowered: dto.targetUserId };
    });
  }

  @SubscribeMessage(ClientEvent.INVITE_TO_SPEAK)
  onInviteToSpeak(@ConnectedSocket() client: Socket, @MessageBody() body: unknown) {
    return respond(() => {
      const dto = validateWsPayload(TargetPeerDto, body);
      const meeting = this.signaling.getMeetingForNamespace(dto.meetingId, NAMESPACE);
      const actor = this.signaling.requireParticipant(meeting, client);
      this.moderation.inviteToSpeak(meeting, { role: actor.role, userId: actor.userId }, dto.targetUserId);
  ...
  onReaction(@ConnectedSocket() client: Socket, @MessageBody() body: unknown) {
    return respond(() => {
      ...
      this.broadcaster.emitToMeeting(NAMESPACE, meeting.id, ServerEvent.REACTION, {
        peerId: participant.userId,
        emoji: dto.emoji,
        at: Date.now(),
      });
  ...
  onChatMessage(@ConnectedSocket() client: Socket, @MessageBody() body: unknown) {
    return respond(() => {
      const dto = validateWsPayload(ChatMessageDto, body);
      const meeting = this.signaling.getMeetingForNamespace(dto.meetingId, NAMESPACE);
      const participant = this.signaling.requireParticipant(meeting, client);
      const message = this.chat.addMessage(meeting, participant.userId, participant.displayName, dto.text);
      this.broadcaster.emitToMeeting(NAMESPACE, meeting.id, ServerEvent.CHAT_MESSAGE, message);
      return message;
    });
  }
  ...
  onRequestUnmute(@ConnectedSocket() client: Socket, @MessageBody() body: unknown) {
    return respond(() => {
      const dto = validateWsPayload(TargetPeerDto, body);
      const meeting = this.signaling.getMeetingForNamespace(dto.meetingId, NAMESPACE);
      const actor = this.signaling.requireParticipant(meeting, client);
      this.moderation.requestUnmute(meeting, { role: actor.role, userId: actor.userId }, dto.targetUserId);
  ...
  onKick(@ConnectedSocket() client: Socket, @MessageBody() body: unknown) {
    return respond(() => {
      ...
      const meeting = this.signaling.getMeetingForNamespace(dto.meetingId, NAMESPACE);
      const actor = this.signaling.requireParticipant(meeting, client);
      this.moderation.removeParticipant(meeting, { role: actor.role, userId: actor.userId }, dto.targetUserId);
  ...
  onPromote(@ConnectedSocket() client: Socket, @MessageBody() body: unknown) {
    return respond(() => {
      ...
      const meeting = this.signaling.getMeetingForNamespace(dto.meetingId, NAMESPACE);
      const actor = this.signaling.requireParticipant(meeting, client);
      this.moderation.promote(meeting, { role: actor.role, userId: actor.userId }, dto.targetUserId);
  ...
  onDemote(@ConnectedSocket() client: Socket, @MessageBody() body: unknown) {
    return respond(() => {
      ...
      const meeting = this.signaling.getMeetingForNamespace(dto.meetingId, NAMESPACE);
      const actor = this.signaling.requireParticipant(meeting, client);
      this.moderation.demote(meeting, { role: actor.role, userId: actor.userId }, dto.targetUserId);
  ...
  onLock(@ConnectedSocket() client: Socket, @MessageBody() body: unknown) {
    return respond(() => {
      ...
      const meeting = this.signaling.getMeetingForNamespace(dto.meetingId, NAMESPACE);
      const actor = this.signaling.requireParticipant(meeting, client);
      this.moderation.setLocked(meeting, { role: actor.role, userId: actor.userId }, true);
      return { locked: true };
    });
  }
  ...
  onUnlock(@ConnectedSocket() client: Socket, @MessageBody() body: unknown) {
    return respond(() => {
      ...
      const meeting = this.signaling.getMeetingForNamespace(dto.meetingId, NAMESPACE);
      const actor = this.signaling.requireParticipant(meeting, client);
      this.moderation.setLocked(meeting, { role: actor.role, userId: actor.userId }, false);
      return { locked: false };
    });
  }

  @SubscribeMessage(ClientEvent.END_MEETING)
  onEndMeeting(@ConnectedSocket() client: Socket, @MessageBody() body: unknown) {
    return respond(() => {
      const dto = validateWsPayload(MeetingIdOnlyDto, body);
      const meeting = this.signaling.getMeetingForNamespace(dto.meetingId, NAMESPACE);
      const actor = this.signaling.requireParticipant(meeting, client);
      if (actor.role !== ParticipantRole.HOST) throw new Error('Only the host can end the meeting');
  ...
  onDisableFeature(@ConnectedSocket() client: Socket, @MessageBody() body: unknown) {
    return respond(() => {
  ...
  onAdmit(@ConnectedSocket() client: Socket, @MessageBody() body: unknown) {
    return respond(() => {
      ...
      const meeting = this.signaling.getMeetingForNamespace(dto.meetingId, NAMESPACE);
      const actor = this.signaling.requireParticipant(meeting, client);
      const entry = this.waitingRoom.admit(meeting, actor.role, dto.targetUserId);
      ...
      if (admittedSocket) {
        this.signaling
          ...
          .catch((err: Error) =>
            admittedSocket.emit(ServerEvent.ERROR, { message: err.message ?? 'Failed to join after admission' }),
          );
      }
