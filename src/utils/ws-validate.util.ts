 ...
 */
export function validateWsPayload<T extends object>(cls: ClassConstructor<T>, payload: unknown): T {
  if (typeof payload !== 'object' || payload === null) {
    throw new WsException('Payload must be an object');
  }

  const instance = plainToInstance(cls, payload, { excludeExtraneousValues: false });
